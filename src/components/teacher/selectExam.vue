...
export default {
  ...
  methods: {
    edit(examCode) { //编辑试卷
      ...
      this.$axios(`/teacher/exam/${examCode}`).then(res => { //根据试卷id请求后台
        if (res.data.code == 200) {
          this.form = res.data.data.examData
        }
      })
    },
    ...
    deleteRecord(examCode) {
      this.$confirm("确定删除该记录吗，该操作不可逆！！！", "删除提示", {
        ...
        // 确认删除
        return this.$axios({
          url: `/teacher/exam/delete/${examCode}`,
          method: 'post',
        });
      }).then(res => {
        if (res.data.code == 200) {
...
// 如果当前页删除后没有数据了，手动将当前页码减一
    ...
    getExamInfo() { //分页查询所有试卷信息
      this.$axios(`/teacher/exams/${this.pagination.current}/${this.pagination.size}`).then(res => {
