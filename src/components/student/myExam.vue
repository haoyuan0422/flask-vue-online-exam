...
<template>
  <div id="myExam">
    <section class="remind">
      <span class="msg2">点击试卷名称即可进入考试页面</span>
      <!-- <span>管理员账号：9527</span>
      <span>教师账号：20081001</span>
      <span>学生账号：自己的学号</span>
...
export default {
  ...
  methods: {
    ...
    search() {
      ...
      this.$axios(`/exams/${this.pagination.current}/${this.pagination.size}/${inputValue}`).then(res => {
        if(res.data.code == 200) {
          let allExam = res.data.data.pagination.records
