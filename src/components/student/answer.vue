        ...
        <div class="left" v-if="slider_flag">
          ...
          <div class="l-bottom">
            <div class="item">
              <p>选择题部分</p>
              <ul>
                <li v-for="(list, index1) in topic['choices']" :key="index1">
                  <a href="javascript:;" @click="change(index1)"
                    :id="index1"
                    :class="{ 'border': index == index1 && currentType == 1, 'bg': bg_flag && topic['choices'][index1].isClick == true }">
                    <span :class="{ 'mark': topic['choices'][index1].isMark == true }"></span>
                    {{ index1 + 1 }}
                  </a>
                </li>
              </ul>
            ...
            <div class="item">
              <p>填空题部分</p>
              <ul>
                <li v-for="(list, index2) in topic['fills']" :key="index2">
                  <a href="javascript:;" @click="fill(index2)"
                    :class="{ 'border': index == index2 && currentType == 2, 'bg': fillAnswer[index2][3] == true }"><span
                      :class="{ 'mark': topic['fills'][index2].isMark == true }"></span>{{ topicCount[0] + index2 + 1
                      }}</a>
                </li>
              </ul>
            </div>
            <div class="item">
              <p>判断题部分</p>
              <ul>
                <li v-for="(list, index3) in topic['judges']" :key="index3">
                  <a href="javascript:;" @click="judge(index3)"
                    :id="'judge'+index3"
                    :class="{ 'border': index == index3 && currentType == 3, 'bg': bg_flag && topic['judges'][index3].isClick == true }"><span
                      :class="{ 'mark': topic['judges'][index3].isMark == true }"></span>{{ topicCount[0] + topicCount[1]
                        + index3 + 1 }}</a>
                </li>
              </ul>
            </div>
            <div class="final" @click="confirm()">结束考试</div>
          </div>
        </div>
      </transition>
      <!--右边选择答题区-->
      <transition name="slider-fade">
        <div class="right">
          <div class="title">
            ...
            <span>全卷共{{ topicCount[0] + topicCount[1] + topicCount[2] }}题 <i class="iconfont icon-time"></i><b
                style="color: red;">{{ showHour
                }}:{{ showMinute }}:{{ showSecond }}</b></span>
          </div>
          <div class="content">
            ...
            <div class="fill" v-if="currentType == 2">
              ...
              <div class="analysis" v-if="isPractice">
                <ul>
                  ...
                  <li><el-tag>题目解析：</el-tag></li>
                  <li>{{ topic['fills'][index].analysis == null ? '暂无解析' : topic['fills'][index].analysis }}</li>
                </ul>
              </div>
            </div>
            <div class="judge" v-if="currentType == 3">
              ...
              <div class="analysis" v-if="isPractice">
                <ul>
                  <li> <el-tag type="success">正确答案：</el-tag><span class="right">{{ topic['judges'][index].answer }}</span>
                  </li>
                  <li><el-tag>题目解析：</el-tag></li>
                  <li>{{ topic['judges'][index].analysis == null ? '暂无解析' : topic['judges'][index].analysis }}</li>
                </ul>
              </div>
            </div>
          </div>
...
export default {
  ...
  watch: {
    hour(newVal) {
      if (this.hour < 10)
        this.showHour = "0" + `${this.hour}`
      else
        this.showHour = `${this.hour}`
    },
    minute(newVal) {
      if (newVal == -1) {
        this.minute = 59
  ...
  methods: {
    ...
    getExamData() { //获取当前试卷所有信息
      ...
      this.$axios(`/exam/${examCode}`).then(res => {  //通过examCode请求试卷详细信息
        this.examData = { ...res.data.data.examData } //获取考试详情
        if(this.$cookies.get("source")!==null)
          if(this.$cookies.get("source")!==this.examData.source){
            this.removeAllCookies()
            location.reload()
          }
        this.index = 0
        this.time = this.examData.totalTime //获取分钟数
        let paperId = res.data.data.paperId
        this.$axios(`/paper/${paperId}`).then(res => {  //通过paperId获取试题题目信息
          this.topic = { ...res.data.data }
          console.log(this.topic)
          let reduceAnswer = this.topic['choices'][this.index]
          ...
          let dataInit = this.topic['choices']
          this.number = 1
          this.showQuestion = dataInit[0].question
          this.showAnswer = dataInit[0]
        })
      })
      return new Promise((resolve) => {//异步处理,使得showTime能得到Time
        setTimeout(() => {
          console.log('getExamData has been resolved');
          resolve();
        }, 500);
      });
    },
    change(index) { //选择题
      ...
      this.currentType = 1
      let len = this.topic['choices'].length
      if (this.index < len) {
        if (this.index <= 0) {
          this.index = 0
        }
    ...
    fill(index) { //填空题
      let len = this.topic['fills'].length
      this.currentType = 2
      this.index = index
      if (index < len) {
        if (index < 0) {
          index = this.topic['choices'].length - 1
          this.change(index)
        } else {
          console.log(`总长度${len}`)
          console.log(`当前index:${index}`)
          this.title = "请在横线处填写答案"
          let Data = this.topic['fills']
          console.log(Data)
    ...
    judge(index) { //判断题
      let len = this.topic['judges'].length
      this.currentType = 3
      this.index = index
      if (this.index < len) {
        if (this.index < 0) {
          this.index = this.topic['fills'].length - 1
          this.fill(this.index)
        } else {
          console.log(`总长度${len}`)
          console.log(`当前index:${this.index}`)
          this.title = "请作出正确判断"
          let Data = this.topic['judges']
          console.log(Data)
          this.showQuestion = Data[index].question //获取题目信息
    ...
    mark() { //标记功能
      switch (this.currentType) {
        case 1:
          this.topic['choices'][this.index]["isMark"] = true //选择题标记
          break
        case 2:
          this.topic['fills'][this.index]["isMark"] = true //填空题标记
          break
        case 3:
          this.topic['judges'][this.index]["isMark"] = true //判断题标记
      }
    },
    commit() {
      this.removeAllCookies()
      ...
      topic1Answer.forEach((element, index) => {//循环每道选择题根据选项计算分数
        ...
        if (element != null) {
          ...
          if (right == this.topic['choices'][index].rightAnswer) { // 当前选项与正确答案对比
            finalScore += this.topic['choices'][index].score // 计算总分数
          }
          console.log(right, this.topic['choices'][index].rightAnswer)
        }
      ...
      fillAnswer.forEach((element, index) => { //此处index和 this.index数据不一致，注意
        element.forEach((inner) => {
          if (this.topic['fills'][index].answer.includes(inner)) { //判断填空答案是否与数据库一致
            console.log("正确")
            finalScore += this.topic['fills'][this.index].score
          }
        })
      });
      /** 计算判断题总分 */
      ...
      topic3Answer.forEach((element, index) => {
        ...
        if (right == this.topic['judges'][index].answer) { // 当前选项与正确答案对比
          finalScore += this.topic['judges'][index].score // 计算总分数
        }
      })
    ...
    showTime() {
      // 对应分钟转时分秒
      console.log(this.$cookies.get("second"))
      this.$cookies.get("hour") == null ? this.hour = Math.floor(this.time / 60) : this.hour = this.$cookies.get("hour") // 时
      this.$cookies.get("minute") == null ? this.minute = this.time % 60 : this.minute = this.$cookies.get("minute") // 分
      this.$cookies.get("second") == null ? this.second = 0 : this.second = this.$cookies.get("second") // 秒
      if (this.$cookies.get("second") !== null) {
        this.second -= 1
        if (this.second < 0) {
          this.minute -= 1;
          this.second = 59
      ...
      const timerId = setInterval(() => {
        this.second -= 1;
        if (this.hour === 0 && this.minute === 5 && this.second === 0) {
          this.$message.error('考生注意,考试时间还剩5分钟!!!');
        }

        if (this.hour <= 0 && this.minute <= 0 && this.second <= 0) {
          // 停止定时器
          clearInterval(timerId);
          this.$alert('考试时间已到，请交卷！', '警告', {
            confirmButtonText: '确定',
            showClose: false, // 隐藏右上角关闭按钮（×号）
            closeOnClickModal: false, // 点击模态框外部不关闭
            closeOnPressEscape: false, // 按下 ESC 键不关闭
            callback: action => {
              if (action === 'confirm') {
    ...
    saveData() {
      const data = setInterval(() => {
        this.$cookies.set("hour", this.hour)
        this.$cookies.set("minute", this.minute)
        this.$cookies.set("second", this.second)
        this.$cookies.set("showHour", this.showHour)
        this.$cookies.set("showMinute", this.showMinute)
        this.$cookies.set("showSecond", this.showSecond)
        this.$cookies.set("currentType", this.currentType)
        this.$cookies.set("radio", this.radio)
        this.$cookies.set("index", this.index)
        ...
        this.$cookies.set("source",this.examData.source)
        console.log(this.$cookies.get("fillAnswer"))
      }, 1000)
      this.intervalIds.push(data);
    },
    removeAllCookies() {
      const cookieNames = this.$cookies.keys()
      cookieNames.forEach(name => {
        if (!(name == "cname" || name == "cid" || name == "role")){
          console.log(name)
    ...
    initData() {
      if (this.$cookies.get("currentType") !== null)
        this.currentType = this.$cookies.get("currentType")
      if (this.$cookies.get("radio") !== null)
        this.radio = this.$cookies.get("radio")
      if (this.$cookies.get("index") !== null)
        this.index = this.$cookies.get("index")
      ...
      if (this.$cookies.get("judgeAnswer") !== null){
        ...
        const judgeArray = this.$cookies.get("judgeAnswer")
        for(let i=0;i<judgeArray.length;i++){
          if(judgeArray[i]!==null){
            console.log(i)
            document.getElementById('judge'+i).setAttribute("class","bg")
            this.topic['judges'][i].isClick = true
            this.bg_flag = true
          }
        }
      }
      if (this.$cookies.get("topic1Answer") !== null){
        ...
        const AnswerArray = this.topic1Answer
        for(let i=0;i<AnswerArray.length;i++){
          if(AnswerArray[i]!==null){
            console.log(i)
            document.getElementById(i).setAttribute("class","bg")
            this.topic['choices'][i].isClick = true
            this.bg_flag = true
          }
        }
      }
